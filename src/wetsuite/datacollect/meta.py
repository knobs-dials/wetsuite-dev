'''
    ...
    This should probably be moved to helpers/
'''
import re, urllib.parse


def parse_jci(s: str):
    """ Takes something in the form of jci{version}:{type}:{BWB-nummer}{key-value}*
          e.g. jci1.31:c:BWBR0012345&g=2005-01-01&artikel=3.1
        The below is robust to a few non-standard things I've seen in real use.
        ...
        Precise interpretation, and generation, is a little more work in that versions made small semantic changes.
    """
    ret = {}
...
CELEX_COUNTRIES = [ 'BEL', 'DEU', 'FRA', 'CZE', 'ESP', 'PRT', 'AUT', 'CYP', 'BGR', 'EST', 'FIN', 'GBR', 'HUN', 'IRL', 'LTU', 'MLT', 'LVA', 'SVN', 'SWE', 'GRC,' 'POL', 'DNK', 'ITA', 'LUX', 'NLD', 'SVK', 'ROU', 'HRV' ]
...
def parse_celex(celex: str):
    ...
    if addition not in (None,''):
        if addition[:3] in CELEX_COUNTRIES:
            nattrans = addition
        elif addition[0] == '-':
            specdate = addition
        else:

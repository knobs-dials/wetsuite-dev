...
def test_uri_dict():
    ...
    assert uri_dict({'name':'Zim', 'chr':'\u2222'}, astype=str)              ==  'chr=%E2%88%A2&name=Zim'
    assert uri_dict({'name':'Zim', 'chr':'\u2222'}, astype=bytes)            == b'chr=%E2%88%A2&name=Zim'
    assert uri_dict({'name':'Zim', 'chr':'\u2222'}, join=b';', astype=str)   ==  'chr=%E2%88%A2;name=Zim'
    assert uri_dict({'name':'Zim', 'chr':'\u2222'}, join=';',  astype=str)   ==  'chr=%E2%88%A2;name=Zim'
